...
USB 웹캠은 `usb`, 카메라 모듈은 `module` 인자를 사용합니다. 업데이트 후 Chromium에서 새로고침하세요.

설정 → 인식 미세조정에서 전처리 **회색조**, 글 배치 **책 전체·자동**, 처리해상도 **2600 px**로 시작하세요. 글 위주 한 문단은 해당 글 배치를, 수식/그림 사이 글은 흩어진 글 배치를 비교할 수 있습니다. 작은 글씨는3200 px를 시험하되 더 느려질 수 있습니다. USB 촬영 해상도는 브라우저에 요청한 값이므로 실제 카메라 지원에 따라 낮을 수 있습니다. 해상도 변경 후 중지→시작하면 촬영 요청에도 반영됩니다. Pi 모듈은2592×1944로 촬영하며 약2.2초 안정화 시간을 줍니다.

‘인식한 글 확인’에서 분류 전 OCR 글자를 확인합니다. ‘최근 촬영 원본 저장’으로 원본 사진을 내려받아 초점과 작은 글자를 확인할 수 있습니다. 신뢰도는 OCR 자체 점수이며 실제 정확도가 아닙니다. 촬영 원본/글자/분류를 비교해 조정해야 하며, 사용자 책 원본 없이 인식률 개선을 보장하지 않습니다.
